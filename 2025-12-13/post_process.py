...
def keep_topk_per_class(bboxes, k=3):
    if len(bboxes) == 0:
        return bboxes
    ...
    for cls in np.unique(bboxes[:, 5]):
        cls_boxes = bboxes[bboxes[:, 5] == cls]
        cls_boxes = cls_boxes[np.argsort(-cls_boxes[:, 4])]  # score desc
        result.extend(cls_boxes[:k])
